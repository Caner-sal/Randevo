  ...
  return (
    <div className="relative">
      ...
      {open && suggestions.length > 0 ? (
        <div className="absolute z-50 mt-1 max-h-56 w-full overflow-auto rounded-xl border border-border bg-popover shadow-lg">
          {suggestions.map((item) => (
            ...
            >
              <div className="text-sm text-popover-foreground">{item.label}</div>
              {item.secondaryLabel ? (
                <div className="text-xs text-muted-foreground">{item.secondaryLabel}</div>
              ) : null}
            </button>
          ))}
        </div>
